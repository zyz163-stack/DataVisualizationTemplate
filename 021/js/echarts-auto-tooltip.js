    ...
    tools.loopShowTooltip = function (chart, chartOption, options) {
        ...
        function autoShowTip() {
            function showTip() {
                ...
                showOtherData(options.dataNum,options.showData);
        ...
        function showOtherData(num,data) {
            var copyIndex;
            copyIndex = dataIndex;
            if(num==1){
                $("#data1_area_name").text(data[dataIndex]["Region"]);
                $("#data1_rate_name").text('工业');
                $("#data1_rate_value").text(parseFloat(parseFloat(data[dataIndex]["ActRate"])*100).toFixed(0)+"%");
                setTimeout(function () {
                    $("#data1_area_name").text(data[copyIndex]["Region"]);
                    $("#data1_rate_name").text('制造业');
                    $("#data1_rate_value").text(parseFloat(parseFloat(data[copyIndex]["ActRate1"])*100).toFixed(0)+"%");
                },"3000");
            }else if(num==2){
                $("#data2_area_name").text(data[dataIndex]["Region"]);
                $("#data2_rate_name").text('工业');
                $("#data2_rate_value").text(parseFloat(parseFloat(data[dataIndex]["ActRate"])*100).toFixed(0)+"%");
                setTimeout(function () {
                    $("#data2_area_name").text(data[copyIndex]["Region"]);
                    $("#data2_rate_name").text('制造业');
                    $("#data2_rate_value").text(parseFloat(parseFloat(data[copyIndex]["ActRate1"])*100).toFixed(0)+"%");
                },"3000");
            }
        }
        // 关闭轮播
